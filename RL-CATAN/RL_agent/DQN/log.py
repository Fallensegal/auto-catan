...
class Log:
    def __init__(self):
        ...
        self.action_counts = [0 for _ in range(TOTAL_ACTIONS)]
        self.random_action_counts = [0 for _ in range(TOTAL_ACTIONS)]

    def logging(self, num_episode):
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=list(range(len(self.action_counts))), y=self.action_counts, mode='markers', name='Action Counts'))
        fig.add_trace(go.Scatter(x=list(range(len(self.random_action_counts))), y=self.random_action_counts, mode='markers', name='Random Action Counts'))
